...
def find_highscore(*, cursor : MySQLCursor, uid : int) -> tuple[str, float]:
    cursor.execute("SELECT highscore_content, highscore_score FROM drawings WHERE uid=%s;", (uid, ))
    data = cursor.fetchall()
    return data[0]

def update_highscore(*, cursor : MySQLCursor, uid : int, highscore_content : str, highscore_score : float) -> bool:
    _, last_highscore_score = find_highscore(cursor=cursor, uid=uid)
    if last_highscore_score < highscore_score:
        return False
    cursor.execute("UPDATE drawings SET highscore_content=%s, highscore_score=%s WHERE uid=%s;", (highscore_content, highscore_score, uid))
    cursor.fetchall()
    return True
...
def create_drawing(*, cursor : MySQLCursor, title : str, author : str, content : str) -> None:
    ...
    cursor.execute("INSERT INTO drawings \
        (uid, title, time_created, time_modified, author, views, likers, content, highscore_content, highscore_score, score, last_score_time) \
